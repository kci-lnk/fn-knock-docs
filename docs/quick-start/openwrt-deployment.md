...
不要给 WAN 转发或放行管理端口。需要公网访问时，只开放当前配置的网关端口，并先完成模式、证书和访问策略配置。全新安装默认可由 fn-knock 管理这一条网关 WAN 入站规则；从较早安装升级时不会自动扩大暴露范围，需要在 LuCI 中明确开启。

### 自动开放网关端口

LuCI 的 `自动开放网关端口` 只管理一条由 fn-knock 拥有的 OpenWrt firewall4 规则：来源区域为 `wan`，协议为 TCP，目标是当前 Go 网关端口，同时适用于 IPv4 和 IPv6。服务重载或端口变化时会同步更新；关闭开关、停止服务或卸载软件包时会移除该规则。

该功能不会开放管理端口或内部服务端口，也不会配置 NAT、DNS、证书或登录策略。若已存在同名但并非 fn-knock 创建的规则，程序会报告冲突而不会覆盖或删除它；防火墙同步失败也不会阻止仅供 LAN 使用的服务启动。使用自定义 firewall zone 或已有集中规则时，应关闭该开关并自行维护端口。

保存规则使用独立的 UCI 会话，不会顺带提交 LuCI 中其他尚未应用的防火墙修改。开启后仍需从移动网络验证域名、证书和认证流程，并在 `网络 → 防火墙` 确认实际规则范围。
...
| 直连模式、宿主机防火墙管理 | 不提供通用管理；仅可选择维护一条网关 WAN 入站规则，其他端口仍使用 OpenWrt 防火墙、VPN 或上级网关控制 |
