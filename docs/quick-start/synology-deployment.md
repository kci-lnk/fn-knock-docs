...
> [!IMPORTANT]
> `2.2.5` 修复了部分 DSM 环境中套件无法启动的问题，`2.2.6` 又加固了冷启动时管理服务与 Go 网关的就绪同步。已经安装旧版但无法稳定启动时，不要先卸载或清空数据目录；下载相同套件架构的 `2.2.6` 或更高版本 SPK，在套件中心手动覆盖安装。正常覆盖升级会保留 `/var/packages/fn-knock-synology/var`。
...
`2.2.6` 的启动流程会等待 Go 网关控制面可用，并在有限总时长内重试暂时性的连接拒绝、超时或 `502 / 503 / 504`；内存设置和完整 Host 规则成功下发后，应用才报告就绪。DSM 生命周期脚本的启动超时会传递给应用，避免脚本已经判定失败而后台仍继续初始化。持续出现 `400`、无效 Host 规则等永久配置错误时不会无限重试，应查看 `fn-knock.log` 中最早的启动同步错误并修正配置。
