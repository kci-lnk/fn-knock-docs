...
生命周期脚本会在停止或重启时终止 fn-knock 的整个独立进程组，并额外检查管理后端与 Go 网关 PID，避免父进程退出后留下子进程占用端口。若套件显示已停止但 `7998` 或 `7999` 仍被旧进程占用，先覆盖安装相同架构的最新 SPK 再从套件中心启动；不要反复安装不同架构的 SPK，也不要直接删除 PID 文件冒充已停止。
