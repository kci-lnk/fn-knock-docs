...
> **推荐使用官网原生 FPK**
>
> 官网提供的标准飞牛 FPK 是 fn-knock 功能最完整的版本。如果设备中显示的是 `敲门 knock Lite`，它是原生非 root 的精简包，不是 Docker 版：支持反向代理、认证、DDNS、证书、WAF、内置隧道和监控，但不提供直连与宿主机防火墙、网络优化、飞牛证书库同步、Web 终端、FN Connect WAF 接入或应用内更新。迁移时先从 Lite 导出 `.knock` 备份并停止或卸载 Lite，再按设备架构安装标准 FPK 后导入；不要同时运行两个版本，以免端口冲突。
