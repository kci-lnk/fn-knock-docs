...
주소 선택은 더 이상 인터페이스가 반환한 목록의 순번에 의존하지 않습니다. 기본 `안정적인 주소 자동 선택`은 현재 주소를 계속 사용할 수 있다면 우선 유지하므로 후보 순서가 바뀌어 레코드가 불필요하게 변경되는 것을 방지합니다. 현재 주소를 사용할 수 없게 된 뒤에야 설정한 선호 주소 또는 안정적인 후보를 선택합니다. 이전 버전에서 저장한 주소 순번은 페이지에서 안정적인 선택 규칙으로 변환되며 저장 후 적용됩니다. 이전 설정에 선택 규칙이 없거나 기존 순번이 무효가 되어도 백엔드는 자동 안정 선택 규칙으로 후보를 선택하므로 순번이 없다는 이유만으로 무인 업데이트가 중단되지는 않습니다.
...
| 설정 | 역할 |
| --- | --- |
| `선호 주소` | 후보에 해당 주소가 있으면 우선 사용하며 주소가 무효가 되어도 다른 일치 항목 선택 가능 |
