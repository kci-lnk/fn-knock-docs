...
`Tencent Cloud DNSPod`를 사용하면 fn-knock가 루트 도메인, 서브도메인, 레코드 유형 및 라인을 기준으로 대상을 먼저 조회합니다. A 또는 AAAA 레코드가 없으면 자동으로 생성하고, 기존 주소와 다르면 업데이트하며, 주소가 같으면 쓰기를 생략합니다. 설정한 라인과 TTL은 생성과 업데이트에 모두 사용됩니다.

로그가 여전히 레코드 조회 단계에서 실패하더라도 먼저 자리 표시자 레코드를 수동으로 만들지 않습니다. 루트 도메인과 서브도메인 분리, 레코드 라인 및 Token 권한을 확인합니다. 실제 조회 오류는 DNSPod 응답을 유지하고 “레코드 없음”은 자동 생성 절차로 넘어갑니다.
...
`인터페이스를 직접 사용`을 선택하면 인터페이스 하나를 명확히 지정합니다. 기본적으로 전역 라우팅이 가능한 공인 후보만 사용합니다. RFC1918 사설 IPv4, IPv6 ULA, 아직 주소 확인 중이거나 충돌 검사에 실패했거나 사용 중단된 주소를 제외하며 임시 / 프라이버시 IPv6 주소도 기본적으로 제외합니다. 후보가 없다면 다른 인터페이스를 선택하거나 공인 IP 감지, 고정 IP 또는 도메인 조회 방식으로 바꿉니다.

내부 DNS, VPN 또는 분할 DNS에 사설 주소를 게시하려면 `사설 주소 허용`을 켭니다. 이 스위치는 현재 대상의 인터페이스 후보에 RFC1918 IPv4와 IPv6 ULA만 추가합니다. 루프백, 링크 로컬, CGNAT 또는 다른 예약 주소는 포함하지 않으며 라우터, 방화벽 또는 공용 DNS의 접근 가능성도 바꾸지 않습니다.
...
사설 주소를 허용해도 명시적으로 지정한 `선호 주소`가 가장 먼저 적용되고 현재 사용 가능한 주소가 그다음입니다. 다시 자동 선택할 때는 공인 후보가 사설 후보보다 우선합니다. 스위치를 끄면 미리보기와 이후 업데이트에서 사설 주소를 즉시 제외합니다. DNS 레코드에 사설 주소가 남아 있다면 직접 업데이트하거나 다음 동기화에서 바뀔 때까지 기다립니다.
...
자동 동기화 간격은 `2`분에서 `1440`분까지 설정할 수 있으며 기본값은 `10`분입니다. 새 간격을 저장하면 다음 동기화부터 적용됩니다. `지금 새로고침`이나 개별 추가 도메인의 `지금 업데이트`를 수동으로 실행할 때는 주기를 기다릴 필요가 없습니다.
...
2. 인터페이스 선택기 미리보기의 주소가 대상 DNS 용도에 맞는지 확인합니다. 공용 레코드는 Docker나 LAN 주소를 잘못 선택하면 안 되며 내부 레코드는 `사설 주소 허용`을 켰는지 확인합니다.
