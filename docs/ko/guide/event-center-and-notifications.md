...
이벤트 페이지는 현재 다음 20가지 시스템 이벤트를 인식합니다.

- 인증: 로그인 성공, 로그아웃, 로그인 실패, 세션 IP 변경
- 보안: 스캐너 차단, 게이트웨이 요청 제한, WAF 차단
...
이벤트 세부 정보에는 유형에 따라 자격 증명, 인증 방식, 세션, IP와 위치, 실패 횟수, 차단 시간, DDNS 주소 변경, Trace ID, WAF 규칙, 터널 PID, 리소스 임계값 등의 필드가 표시됩니다. 문제를 해결할 때 세부 정보를 복사하여 요청 로그, WAF 로그 또는 터널 로그와 대조할 수 있습니다.
