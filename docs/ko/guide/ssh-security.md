...
3. “SSH 보안 설정”을 펼쳐 규칙을 저장한 뒤 “방화벽 동기화”를 실행합니다.

기능 메뉴 스위치는 페이지와 백엔드 기능의 활성화 여부를 제어합니다. 설정 카드의 “SSH 보안 활성화”는 로그, 지역 및 임계값을 기준으로 보호 기능을 실행할지 결정합니다. 설정을 저장하고 방화벽 동기화에 성공해야 허용 CIDR과 활성 차단이 감지된 SSH 포트에 모두 적용됩니다.
...
| SSH 접근 허용 지역 | 기본적으로 비어 있음 | 비어 있으면 지역을 제한하지 않으며 값이 있으면 선택한 지역과 사용자 지정 CIDR만 허용 |
| 사용자 지정 CIDR | 기본적으로 비어 있으며 한 줄에 하나 | VPN, 사무실 공인 인터넷 출구 또는 다른 고정 IPv4/IPv6 네트워크 대역 추가 |
...
2. 저장 후 방화벽을 동기화하고 신뢰할 수 있는 네트워크와 테스트 네트워크에서 각각 SSH 로그인을 확인합니다.
3. 로그인 로그가 사용자, 출발지 IP, 인증 방식 및 포트를 올바르게 식별하는지 확인합니다.
4. 고정 VPN, 사무실 인터넷 출구 및 예비 네트워크를 사용자 지정 CIDR에 추가합니다.
5. 그다음 지역 제한을 추가해 동기화하고 허용되는 출발지와 허용되지 않는 출발지에서 각각 확인합니다.
