...
`.knock`는 비밀번호가 설정된 ZIP 호환 아카이브이지만 서로 다른 인스턴스에서 가져올 수 있도록 아카이브 비밀번호가 fn-knock에 고정되어 있습니다. 따라서 실질적인 기밀성을 제공하지 못합니다. 파일에는 인증서 개인 키, TOTP 시드, 계정 자격 증명 해시, OIDC / DDNS / 알림 / FRP 키가 포함될 수 있습니다.

평문 키 백업과 같은 수준으로 취급합니다.
