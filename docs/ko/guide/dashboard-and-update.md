...
`2.3.0`부터 표준 fnOS FPK의 앱 내 설치는 먼저 SQLite WAL을 주 데이터베이스에 체크포인트하고 빠른 무결성 검사를 실행한 뒤 데이터 디렉터리에 검증된 일관성 스냅샷 `fn-knock.sqlite3.pre-update.bak`를 만듭니다. 사전 검사가 성공하면 이후 쓰기는 일시적으로 더 엄격한 동기화 모드를 사용합니다. 어느 단계든 실패하면 프로그램 버전을 전환하기 전에 설치를 중단합니다.
