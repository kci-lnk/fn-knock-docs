...
표준 fnOS FPK의 앱 내 설치는 먼저 SQLite WAL을 주 데이터베이스에 체크포인트하고 빠른 무결성 검사를 실행한 뒤 데이터 디렉터리에 검증된 일관성 스냅샷 `fn-knock.sqlite3.pre-update.bak`를 만듭니다. 사전 검사가 성공하면 이후 쓰기는 일시적으로 더 엄격한 동기화 모드를 사용합니다. 어느 단계든 실패하면 프로그램 파일을 전환하기 전에 설치를 중단합니다.

정상적인 서비스 중지 시에도 백그라운드 작업이 끝날 때까지 기다린 뒤 WAL을 다시 체크포인트합니다. fnOS, OpenWrt 및 Synology 패키지 스크립트는 SQLite가 정상 종료를 마칠 시간을 제공합니다. 프로세스를 중지할 수 없으면 이전 프로세스가 데이터베이스를 쓰는 동안 파일을 교체하지 않고 업그레이드 또는 제거 작업을 오류로 종료합니다.

업데이트 전 스냅샷은 활성 데이터베이스와 같은 데이터 디렉터리에 있으며 전환 중 위험을 줄이기 위한 것입니다. 독립 백업이 아니며 `.knock` 내보내기, 데이터 디렉터리 백업 또는 시스템 스냅샷을 대신하지 않습니다. 배포 문서에 따라 다른 장치에도 백업을 보관합니다.
...
Synology 패키지는 콜드 부팅 중 게이트웨이 준비 상태와 설정 동기화를 강화합니다. 시작이 불안정한 인스턴스가 현재 시작되지 않더라도 현재 릴리스 채널에서 같은 아키텍처의 최신 SPK를 패키지 센터에서 바로 덮어 설치합니다. 시작을 복구하려고 패키지를 먼저 제거하거나 데이터 디렉터리를 삭제하지 않습니다.
