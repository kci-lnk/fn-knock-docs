...
| `업스트림 오류 시 오류 정보 표시` | 기본값은 `간단히 표시`이며 업스트림 IP, 포트 및 내부 연결 오류를 숨김. `자세히 표시`는 문제 해결 중에만 임시 사용 |
...
`업스트림 오류 시 오류 정보 표시`의 기본값은 `간단히 표시`입니다. 방문자에게 업스트림을 사용할 수 없다는 사실만 알리고 내부 IP, 포트 또는 연결 실패 원인은 노출하지 않습니다. `자세히 표시`는 전체 연결 오류를 반환해 네트워크 토폴로지를 노출할 수 있습니다. 통제된 환경에서 짧게 문제를 해결할 때만 사용하고 완료 후 `간단히 표시`로 되돌립니다.
