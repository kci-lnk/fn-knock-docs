...
| 子域规则组 ID / 临时凭据状态 | 高级认证命中时，确认具体规则组以及凭据是已签发、已续期还是已复用 |
...
启用子域高级认证后，鉴权结果 `子域规则放行` 表示请求由高级认证临时凭据通过。详情中的规则组 ID 可定位触发配置；凭据状态为 `已签发` 表示本次新建凭据，`已续期` 表示刷新空闲有效期，`已复用` 表示继续使用现有凭据。规则行为见[子域高级认证](/guide/advanced-auth)。
