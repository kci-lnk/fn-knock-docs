...
OpenAPI 契约为全部管理接口提供简体中文的分类说明、接口摘要、行为边界和响应说明，部分复杂接口还包含请求示例、响应示例与 Schema 字段说明。这些内容同时存在于 Swagger UI 和 `/docs/json`，代码生成器与 AI Agent 也能读取；接口路径、方法和字段名仍保持代码中的原始形式。

契约中的说明文本不会随管理后台界面语言切换，目前以简体中文提供。调用前应同时阅读操作说明与 Schema，不要只根据接口名称猜测写操作的副作用。
