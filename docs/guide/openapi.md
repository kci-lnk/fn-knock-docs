...
从 `2.3.0` 开始，OpenAPI 契约为全部管理接口补充了简体中文的分类说明、接口摘要、行为边界和响应说明，部分复杂接口还包含请求示例、响应示例与 Schema 字段说明。这些内容同时存在于 Swagger UI 和 `/docs/json`，代码生成器与 AI Agent 也能读取；接口路径、方法和字段名仍保持代码中的原始形式。
