...
当前版本不接管已开启 fnOS `强制 HTTPS` 的明文 FN Connect 链路；检测到该设置时不会启用重定向。若端口不可访问、规则写入失败或本地 WAF 入口异常，系统采用 fail-open：清理重定向并停止对应入口，优先避免 FN Connect 整体中断。此时应按状态错误修复环境，不能把“远程仍可访问”误认为 WAF 仍在生效。
