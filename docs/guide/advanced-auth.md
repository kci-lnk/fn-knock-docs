...
子域高级认证为一条已开启 `要求登录` 的 HTTP / HTTPS 业务 Host 增加条件放行规则。请求首次命中规则时，系统签发仅限当前 Host 的临时凭据；没有命中时，仍按正常登录、会话和服务范围处理。
...
规则只在签发临时凭据时匹配。凭据签发后，在有效期内访问当前 Host 的其他路径或请求方法时，不会要求每次重新满足原条件。因此，路径、Header 或 Query 条件不能当作持续的逐请求访问控制。

例如，规则只匹配 `/health` 并不表示只开放 `/health`；首次命中后，当前 Host 会在临时凭据有效期内整体放行。需要只开放少量固定路径时，使用[路径响应](/guide/gateway-path-response)、拆分独立 Host，或在上游应用中继续执行权限检查。
...
最长使用时间不能短于空闲失效时间。修改规则、有效时间或启用状态并保存后，系统会轮换策略版本，已有临时凭据随即失效；关闭高级认证时会保留规则草稿，便于之后重新启用。
...
临时凭据只在首次命中且当前没有可复用凭据时签发。系统对同一 Host、同一客户端 IP 限制为每 60 秒最多签发 10 个，对单个 Host 的总签发量限制为每 60 秒 1000 个；单个 Host 最多保留 100000 个活动凭据。超过签发频率时返回 `429 Too Many Requests` 和 `Retry-After: 60`。共享代理出口会让多个客户端计入同一来源，因此前置代理场景必须先确认真实客户端 IP 识别正确。
...
| 请求 Header | 存在、不存在、等于、不等于、包含、开头、结尾及对应否定、RE2 正则 | 先填写 Header 名称；值默认区分大小写 |
...
3. 查看 `子域规则组 ID` 是否为预期规则组，`临时凭据状态` 是否为 `已签发`、`已续期` 或 `已复用`。
4. 再从不符合条件的来源测试，确认请求回到正常登录流程。
5. 修改规则并保存，确认旧浏览器中的临时凭据立即失效。
