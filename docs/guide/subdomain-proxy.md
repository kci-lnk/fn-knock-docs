...
Target 包含路径时，网关会保留这段基础路径并在其后拼接访客请求路径，可用于 fnOS 相册、音乐等挂载在子路径下的服务。保存后必须实际验证首页、静态资源、重定向、Cookie 和 WebSocket；如果上游只支持部署在根路径，单纯填写基础路径不能替它完成应用侧改写。标题与图标采集会使用完整 Target，并正确保留显式端口。
