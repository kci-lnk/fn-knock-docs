...
3. 展开“SSH 安全配置”，保存规则后执行“同步防火墙”。

功能菜单开关控制页面和后台功能是否启用；配置卡片中的“启用 SSH 安全”决定是否按日志、地区和阈值执行防护。只有保存配置并成功同步防火墙后，允许 CIDR 和活动封锁才会完整下发到检测到的 SSH 端口。
...
| 允许访问 SSH 的地区 | 默认空 | 空时不限制地区；非空时只允许所选地区和自定义 CIDR |
| 自定义 CIDR | 默认空，每行一条 | 补充 VPN、办公公网出口或其他固定 IPv4/IPv6 网段 |
...
2. 保存后同步防火墙，从可信网络和测试网络分别验证 SSH 登录。
3. 查看登录日志是否正确识别用户、来源 IP、认证方式和端口。
4. 将固定 VPN、办公出口和备用网络加入自定义 CIDR。
5. 再添加地区限制并同步，从允许与不允许的来源各验证一次。
