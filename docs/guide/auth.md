...
- TOTP 是一个可命名的身份凭据，可限制其可访问的子域和协议映射；关联的 Passkey、QQ 和其他 OIDC 登录会继承该范围。
...
- 服务范围受限的登录凭据不会创建可放行任意 Host 或原始端口的通用自动 IP 授权，以免来源 IP 扩大凭据范围。这包括受限的 TOTP、其关联 Passkey / OIDC，以及受限服务范围的账号密码账号；它们不适合用作直连模式的自动放行凭据。
