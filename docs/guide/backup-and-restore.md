...
运行日志、事件、会话、临时授权、登录退避、锁、流量统计、WAF 统计、隧道运行状态、DDNS 最近一次地址及其他短期运行数据会被排除。下载的 FRP / Cloudflared / `acme.sh` 文件、Cloudflare API Token 与 Tunnel Token、宿主机防火墙状态、外部 DNS 记录和上游应用数据也不在应用备份内。恢复后需要重新连接 Cloudflare API 或重新填写手动 Tunnel Token；远端 Cloudflare 资源不会因导入备份而自动删除或重建。
