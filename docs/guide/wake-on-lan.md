...
`远程唤醒` 用 Magic Packet 启动已关机或休眠的局域网设备。fn-knock 可以直接向本机所在局域网广播，也可以通过另一台 fn-knock 的内置 Relay 跨网络广播；已登录用户还可以从网关传送门打开精简唤醒页。

Wake-on-LAN 只能发出唤醒信号，不能替代设备自身的电源管理。目标主板、网卡、固件和操作系统必须已经启用 WOL，并且设备断电后仍需保留网卡供电。
...
使用 `全部范围` 的凭据自动包含该入口；使用 `自定义范围` 时，需要在 `认证配置 → 权限` 中勾选 `内置远程唤醒页面`。隐藏传送门快捷入口不会删除设备，也不会影响管理员、Relay 或第三方平台触发，但会禁止通过公共认证 API 打开和操作该内置页面。
...
每次唤醒完成都会写入 `远程唤醒完成` 事件，记录目标、投递方式、来源（管理员、传送门、点灯科技或巴法云）、结果和延迟，可在事件中心创建通知规则。事件表示广播流程的结果，不等同于设备已经启动；应结合后续在线状态判断。

设备、Relay 和非敏感集成设置属于应用配置。Relay PSK、点灯设备密钥和巴法云私钥保存在当前安装绑定的加密凭据目录中，不包含在 `.knock` 应用备份里。迁移或恢复后应重新配对 Relay、重新填写第三方凭据，并验证广播地址、设备 IP、UDP 端口和防火墙是否仍适用于新环境。
...
8. 第三方平台无响应时，查看连接状态和最近错误，确认出站 HTTPS / MQTT TLS、凭据和主题，再重新保存设备。
