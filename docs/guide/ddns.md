...
地址选择不再依赖网卡返回列表中的序号。默认的 `自动选择稳定地址` 会优先继续使用当前仍可用的地址，避免候选顺序变化导致记录跳动；当前地址失效后，再选择配置的优先地址或稳定候选。旧版本保存的地址序号会在页面中转换为稳定选择规则，保存后生效；旧配置没有选择规则或旧序号已经失效时，后台也会按自动稳定规则选择候选，不会仅因缺少序号而中断无人值守更新。
...
| 设置 | 作用 |
| --- | --- |
| `优先地址` | 候选中存在该地址时优先使用；地址失效后仍可选择其他匹配项 |
