...
已有配置中的同端口本机回环规则可能使协议映射自动停用。此时保留停用状态，在协议映射页面删除或修正无效规则，再回到 `系统设置 → 功能` 重新开启。不要通过反复同步网关尝试带病启动。
...
流量详情中的 `活跃 IP` 会列出统计窗口内连接过该映射的客户端地址、最近活跃时间、连接数和属地。可刷新列表，并把确认异常的公网 IP 加入或移出[通用黑名单](/guide/general-blacklist)。私网或本机地址会标记为本地来源；加入黑名单前先排除 NAT 共享出口、前置代理和可信运维地址。
