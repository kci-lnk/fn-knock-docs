...
从 `2.3.0` 开始，新建协议映射只保存端口和 Target，初始保持停用。保存后在该规则的更多操作中执行 `检测`；系统会主动连接 Target，只有得到高置信度、且支持严格校验的服务识别结果后，才会启用映射并把校验模式切换为 `严格协议校验`。检测失败、结果不明确或识别到的服务不支持当前传输协议时，规则继续保持停用。

上游认证可能让 HTTP 探测只能看到 `401`，无法可靠区分普通 HTTP 与 WebDAV 等具体服务。此时从更多操作选择 `指定服务类型`，核对 Target 实际运行的服务，再执行 `确认并启用校验`。选择错误会使合法连接因协议不符而被拒绝；清除手工指定的服务类型也会关闭严格校验并停用映射。

修改一条规则的 Target 会让原识别结果失效，并自动停用该规则；需要对新 Target 重新检测或指定服务类型。保持传输协议、对外端口和 Target 不变，只修改备注或登录要求时，已验证的服务识别结果会保留；修改协议或端口会按新规则处理，同样需要重新检测或指定服务类型。

升级前已经存在、尚未带服务识别信息的规则会显示为旧规则或未启用严格校验，不会只因升级而自动改写为某种服务。应逐条确认 Target 后执行检测；在完成确认前不要通过编辑 Target 来试探服务，因为 Target 变化会按新规则停用映射。
