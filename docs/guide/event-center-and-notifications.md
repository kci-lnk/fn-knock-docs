...
事件级别分为信息、注意、错误和严重；来源系统分为管理后台、认证代理和系统监控。
...
事件详情根据类型显示凭据、认证方式、会话、IP 与属地、失败次数、封锁时间、DDNS 地址变化、Trace ID、WAF 规则、隧道 PID、资源阈值等字段。网关可见性拦截还会记录请求 Host、路径、方法，以及生效的是网关全局还是 Host 自定义范围。排障时可复制详情，与请求日志、WAF 日志或隧道日志对照。
