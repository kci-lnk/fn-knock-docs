...
- 確認 TOTP、密碼、Passkey、OIDC 或 LDAP 至少有一種可正常登入。
- 備份目前的模式、路由及防火牆設定。
...
限制服務範圍的登入憑證不會建立自動 IP 授權。這包含受限的 TOTP、與其綁定的 Passkey / OIDC / LDAP，以及限制服務範圍的帳號密碼帳號；如此可避免受限憑證透過來源 IP 擴大存取範圍。直連授權應使用未限制服務範圍的憑證，或手動加入目前的公網 IP / CIDR；之後再從外部網路重新驗證連接埠。
