...
3. 展開「SSH 安全性設定」，儲存規則後執行「同步防火牆」。

功能選單開關會控制頁面與後台功能是否啟用；設定卡片中的「啟用 SSH 安全性」則決定是否依 Log、地區與臨界值執行防護。只有儲存設定並成功同步防火牆後，允許 CIDR 與有效封鎖才會完整下發至偵測到的 SSH 連接埠。
...
| 允許存取 SSH 的地區 | 預設空白 | 空白時不限制地區；非空白時只允許所選地區與自訂 CIDR |
| 自訂 CIDR | 預設空白，每行一筆 | 補上 VPN、辦公室公網出口或其他固定 IPv4／IPv6 網段 |
