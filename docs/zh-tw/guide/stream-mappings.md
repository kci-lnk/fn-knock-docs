...
從 `2.3.0` 起，新建通訊協定映射只會先儲存連接埠與 Target，初始維持停用。儲存後請從該規則的更多操作執行 `偵測`；系統會主動連線 Target，只有取得高信賴度、且支援嚴格驗證的服務識別結果時，才會啟用映射並將驗證模式切換為 `嚴格通訊協定驗證`。偵測失敗、結果不明確，或識別出的服務不支援目前 Transport 時，規則會繼續保持停用。

上游驗證可能讓 HTTP Probe 只能看到 `401`，無法可靠區分一般 HTTP 與 WebDAV 等具體服務。此時請從更多操作選擇 `指定服務類型`，核對 Target 實際執行的服務，再執行 `確認並啟用驗證`。選錯類型會讓合法連線因通訊協定不符而遭拒；清除手動指定的服務類型也會關閉嚴格驗證並停用映射。

修改規則的 Target 會使原本的服務識別結果失效，並自動停用該規則；必須對新的 Target 重新偵測或指定服務類型。只有在 Transport、對外連接埠與 Target 都不變，僅修改備註或登入要求時，已驗證的服務識別結果才會保留；修改 Transport 或連接埠會按新規則處理，同樣需要重新偵測或指定服務類型。

升級前已存在、尚未包含服務識別資訊的規則，可能顯示為舊規則或未啟用嚴格驗證；系統不會只因升級就自動指定服務類型。請逐筆確認 Target 後執行 `偵測`。確認完成前，不要只為測試服務而編輯 Target，因為 Target 變更會依新規則停用映射。
