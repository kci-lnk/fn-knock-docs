...
OpenAPI 契約為所有管理操作提供簡體中文的分類說明、操作摘要、行為邊界與 Response 說明；部分複雜操作也包含 Request Example、Response Example 與 Schema 欄位說明。這些內容同時存在於 Swagger UI 及 `/docs/json`，程式碼產生器與 AI Agent 也能讀取；Path、Method 與欄位名稱仍維持程式碼中的原始形式。
