...
Target にパスを含めると、そのベースパスを維持して訪問者のリクエストパスを後ろへ追加します。サブパスに配置された fnOS Photos や Music などで利用できます。保存後はトップページ、静的リソース、リダイレクト、Cookie、WebSocket を確認してください。ルート配置しかサポートしないアプリを、Target のパス指定だけで対応させることはできません。タイトルとアイコンの取得には完全な Target と明示ポートが使われます。
