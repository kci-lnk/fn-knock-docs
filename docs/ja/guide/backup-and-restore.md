...
実行ログ、イベント、セッション、一時アクセス許可、ログインのバックオフ状態、ロック、トラフィック統計、WAF 統計、トンネルの実行状態、DDNS が最後に取得したアドレス、その他の短期的な実行データは除外されます。ダウンロードした FRP / Cloudflared / `acme.sh` のファイル、Cloudflare API Token と Tunnel Token、ホストのファイアウォール状態、外部 DNS レコード、アップストリームアプリのデータも含まれません。復元後は Cloudflare API を再接続するか、手動 Tunnel Token を再入力してください。バックアップのインポートによって Cloudflare のリモートリソースが自動削除または再作成されることはありません。
