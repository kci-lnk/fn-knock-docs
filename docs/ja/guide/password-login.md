...
- アカウントのサービススコープは、Host へのアクセス時に引き続き適用されます。
...
`認証設定 → パスワードログイン用アカウント` でアカウントを作成し、パスワードと、必要に応じてアクセスできるサブドメインを設定します。
...
アカウントが一部のアプリだけへアクセスする場合は、権限を `カスタム範囲` に切り替え、必要な Host だけを選択します。カスタム範囲が空の場合、保護対象の入口には一切アクセスできません。組み込みの選択ページは個別に選択する必要があります。サービススコープが制限されたアカウントには、送信元に対する許可によって権限が拡大しないよう、ログイン後の IP アクセス許可が自動付与されません。
...
- ログインには成功するが、特定のサブドメインへアクセスできない：アカウントのサービススコープと、マッピングのアクセスポリシーを確認します。
