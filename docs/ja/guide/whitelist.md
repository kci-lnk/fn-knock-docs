...
地域から CIDR を選択すると、独立した地域ホワイトリストグループとして保存され、すべてのアドレス範囲が通常のホワイトリスト一覧へ展開されることはありません。接続先の CIDR データベースに通信事業者データがある場合は、China Telecom、China Unicom、China Mobile で地域を絞り込むこともできます。
...
CNAME はシステムの名前解決結果に応じて、実際に許可する範囲を更新します。自身で管理し、名前解決結果を予測できるドメインだけを使用してください。地域や通信事業者の CIDR グループは、IP 位置情報データに依存します。大まかに範囲を狭める用途には適していますが、高精度な本人確認の唯一の手段には向きません。CIDR データベースに通信事業者データがない場合でも、通常の地域やカスタム CIDR は引き続き使用できます。
