...
タスクまたは自動更新の実行中は、一覧の操作が一時的にロックされます。タスクログには、DNS 認証情報、DNS API のレート制限、ACME の申請回数制限など、確認すべき方向が表示されます。タスクを停止すると、現在実行中の `acme.sh` プロセスが終了し、タスクは停止済みになります。後で改めて実行してください。
...
- サービス再起動後はもう一度スキャンします。終了済みタスクの残存 Lock は状態確認後に削除されますが、停止処理中のタスクはロックを維持し、古いプロセスと新しいタスクの重複を防ぎます。
