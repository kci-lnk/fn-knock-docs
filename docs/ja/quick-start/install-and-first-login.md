...
> **公式サイトのネイティブ FPK を推奨**
>
> 公式サイトで配布する fnOS ネイティブ FPK は、fn-knock のすべての機能に対応する最も完全なバージョンです。fnOS のアプリセンター（アプリストア）からインストールした fn-knock は Docker 版であり、ネイティブ FPK ではありません。現在の設定とデータをバックアップしてアプリセンター版をアンインストールした後、上記の公式リンクから機器のアーキテクチャに合うネイティブ FPK をダウンロードし、手動でインストールしてください。ポートが競合するため、両方のバージョンを同時に実行しないでください。
