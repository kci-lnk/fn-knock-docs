...
直接接続モードはホストのファイアウォールに依存し、現在は主に fnOS ネイティブ FPK と OpenWrt で利用します。Docker、Synology DSM 7 SPK、Windows では fn-knock からホストのファイアウォールを管理できないため、この構成は使用できません。
...
fnOS ネイティブ FPK と OpenWrt では、ファイアウォールの実装が異なります。切り替え直後に外部ネットワークから未ログイン状態で接続し、サービスのポートが確実に拒否されることを確認してください。
...
認証用ドメインには、信頼された TLS 証明書を設定します。自動 HTTPS を使えるのは対応するデプロイ方式だけです。Docker と OpenWrt では証明書を別途用意するか、前段に TLS 終端を設けてください。
...
| OpenWrt パッケージ | 対応 | ルールはルーターのファイアウォールへ適用されます |
...
| Docker にこのモードが見当たらない | デプロイ方式の制約です。サブドメインルーティングへ切り替えてください |
