...
| ポート | 役割 | デフォルトの用途 |
| --- | --- | --- |
| `7991` | 管理パネル | Docker、OpenWrt、Linux のデフォルト管理入口。macOS と Windows ではローカルループバックからのみアクセス可能 |
...
| macOS 13+ | ローカル `127.0.0.1:7991` | `7999` | Intel / Apple Silicon ネイティブパッケージ。内部ポートはループバック専用で、macOS ファイアウォールを変更しません |
...
- macOS の管理画面はループバック専用で、`7999` はサービス入口として利用できます。`iptables` や macOS ファイアウォールの管理、直接接続、スマート接続、Web ターミナル、SSH セキュリティには対応せず、プロトコルマッピングのポートは手動で許可します。
...
fnOS ネイティブ FPK の管理画面を開けない場合は、まずデスクトップアイコン、アプリプロセス、`7998` のバックエンドを確認します。Docker または OpenWrt では、`7991`、バインドアドレス、公開ポートを確認します。macOS では `sudo knock status` と `sudo knock logs` を実行し、ローカル Mac の `127.0.0.1` から接続します。サービス用ドメインを開けない場合は、次の順序で切り分けてください。
